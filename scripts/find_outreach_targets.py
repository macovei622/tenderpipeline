...
async def fetch_outreach_targets():
    ...
    if not leads:
        ...
        for idx, target in enumerate(leads, 1):
            ...
            # Специфічні шаблони під кожну ціль
            edrpou = target.get('disqualified_edrpou')
            if edrpou == '31964715': # ПП «РОУДІЗ»
                msg_body = (
                    "Шановна Наталіє Петрівно! Ми проаналізували тендер щодо ремонту вулиці Кічкарівської в Луцьку, де ваша ціна була нижчою за переможця майже на 50 000 грн, але пропозицію відхилили через брак часу на 24-годинне виправлення.\n\n"
                    "Щоб більше не втрачати вигідні лоти, ми пропонуємо два рішення:\n"
                    "1. Чи є у вас тендер, який ви готуєте прямо зараз? Ми безкоштовно проведемо його AI-аудит за 10 хвилин.\n"
                    "2. Якщо зараз нічого немає — ми можемо безкоштовно підключити ПП «РОУДІЗ» до нашого моніторингу. Щойно у вашому регіоні з'явиться новий тендер на ремонт доріг, ми самі надішлемо вам звіт про приховані вимоги замовника. Що скажете?"
                )
            elif edrpou == '42648805': # ПП «БУДОЛІМПСТРОЙ ЛУЦЬК»
                msg_body = (
                    "Шановна Тетяно Дмитрівно! Ми проаналізували тендер на ремонт вулиці Кічкарівської в Луцьку, де ваша ціна була вигіднішою за переможця майже на 50 000 грн, але виправлені документи не встигли подати вчасно. Це прикра технічна помилка дедлайну.\n\n"
                    "Пропонуємо це виправити:\n"
                    "1. Якщо ви готуєте пропозицію на будь-який тендер прямо зараз — ми безкоштовно зробимо його AI-аудит за 10 хвилин.\n"
                    "2. Якщо зараз нічого немає — ми безкоштовно підключимо ПП «БУДОЛІМПСТРОЙ ЛУЦЬК» до нашого моніторингу. Як тільки з'явиться новий тендер у вашому регіоні на ремонт доріг, ми автоматично надішлемо вам звіт про приховані пастки. Цікаво спробувати?"
                )
            elif edrpou == '41731677': # ТОВ «ВЕНТПРОМЕЛЕКТРОМОНТАЖ»
                msg_body = (
                    "Шановна Лесю Петрівно! Ми проаналізували тендер на встановлення вентиляційної системи в Житомирі, де ваша ціна була вигіднішою за переможця на 99 900 грн, але вас відхилили через розбіжності по обладнанню «Аеростар».\n\n"
                    "Щоб застрахувати вас від подібного:\n"
                    "1. Чи розглядаєте ви якийсь тендер прямо зараз? Ми безкоштовно проведемо його AI-аудит.\n"
                    "2. Якщо ні — ми готові безкоштовно поставити ТОВ «ВЕНТПРОМЕЛЕКТРОМОНТАЖ» на автоматичний AI-моніторинг. Щойно з'явиться новий відповідний тендер на монтаж вентиляції у вашому регіоні — ми одразу надішлемо вам звіт про приховані вимоги."
                )
            elif edrpou == '45795996': # ТОВ «ТЕП КЕПІТАЛ»
                msg_body = (
                    "Шановний Олегу Юрійовичу! Ми проаналізували тендер на капремонт будинку по вул. Табірній у Києві. Ваша ціна була вигіднішою, але пропозицію відхилили одразу через відсутність розрахунків заробітної плати, не давши 24 години на виправлення.\n\n"
                    "Ми розробили AI-сервіс, який страхує від таких випадкових технічних помилок:\n"
                    "1. Чи готуєте ви пропозицію на якийсь тендер прямо зараз? Ми безкоштовно зробимо його аудит перед подачею.\n"
                    "2. Якщо зараз пауза — ми безкоштовно підключимо ТОВ «ТЕП КЕПІТАЛ» до нашого моніторингу. Щойно з'явиться новий тендер на капремонт будівель у вашому регіоні, ми автоматично надішлемо вам звіт про вимоги замовника. Коли зручно поспілкуватися?"
                )
            else:
                msg_body = (
                    f"{greeting_str}! Ми проаналізували нещодавній тендер щодо {target['title'][:60]}... (ID: {target['prozorro_id']}). Ваша ціна була вигіднішою за переможця на {target['diff_amount']:,.0f} грн, але пропозицію відхилили через помилки в довідках.\n\n".replace(",", " ") +
                    "Щоб більше не втрачати вигідні контракти, ми пропонуємо:\n"
                    "1. Чи є у вас тендер у роботі прямо зараз? Ми безкоштовно проведемо його AI-аудит перед подачею.\n"
                    "2. Якщо ні — ми безкоштовно підключимо вашу компанію до нашого AI-моніторингу. Щойно з'явиться відповідний тендер у вашому регіоні — ми автоматично надішлемо вам звіт про приховані пастки замовника. Спробуємо?"
                )
