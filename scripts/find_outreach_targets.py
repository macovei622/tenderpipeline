...
Знаходить тендери по Вінниці та інших регіонах (500к - 50 млн грн, для великих лотів до 150 млн грн), 
                ...
                async with session.get(url, params=params, timeout=30) as resp:
                    ...
                    for t in qual_tenders:
                        ...
                        async with session.get(detail_url, timeout=15) as det_resp:
                            ...
                            # 2. Сума: від 500 тис до 150 млн грн (розширено ліміт для великих інфраструктурних лотів)
                            amount = det_data.get("value", {}).get("amount", 0)
                            if not (500_000 <= amount <= 150_000_000):
                                continue
    ...
    if not leads:
        ...
        for idx, target in enumerate(leads, 1):
            ...
            if edrpou == '31964715': # ПП «РОУДІЗ»
                msg_body = f"Шановна Наталіє Петрівно! Ми проаналізували тендер щодо поточного ремонту вулиці Кічкарівської в Луцьку. Ваша пропозиція була дешевшою за переможця майже на 50 000 грн, але заявку відхилили — ви не встигли повністю виправити зауваження замовника протягом відведених 24 годин. Це прикра, суто технічна причина втрати вигідного контракту, а не питання якості вашої пропозиції. Ми створили сервіс, який відстежує саме такі 24-годинні вимоги замовника і допомагає миттєво зібрати правильний пакет виправлень. Готові безкоштовно розібрати ваш наступний тендер. Коли зручно поговорити?"
            elif edrpou == '42648805': # ПП «БУДОЛІМПСТРОЙ ЛУЦЬК»
                msg_body = f"Шановна Тетяно Дмитрівно! По тому ж тендеру на ремонт вулиці Кічкарівської в Луцьку ваша ціна теж була вигіднішою за переможця майже на 50 000 грн — але виправлені документи не встигли подати вчасно після зауваження замовника. Це та ж сама технічна причина, що зустрічається частіше, ніж здається — не якість роботи, а суто дедлайн. Ми беремо контроль за такими 24-годинними вимогами на себе, щоб це не повторювалось. Готові безкоштовно розібрати ваш наступний тендер?"
            elif edrpou == '41731677': # ТОВ «ВЕНТПРОМЕЛЕКТРОМОНТАЖ»
                msg_body = f"Шановна Лесю Петрівно! Ми проаналізували тендер на встановлення вентиляційної системи в Житомирі. Ваша пропозиція була вигіднішою за переможця на 99 900 грн, але вас відхилили через зауваження щодо технічних характеристик обладнання «Аеростар». Ми допомагаємо звіряти технічні специфікації з паспортами виробників ще до подачі заявки, щоб уникнути подібних розбіжностей. До речі, у вашому випадку скарга на визначення переможця наразі на розгляді Колегії АМКУ. Готові безкоштовно розібрати ваш наступний тендер, щоб такого не повторилось."
            elif edrpou == '45795996': # ТОВ «ТЕП КЕПІТАЛ»
                msg_body = f"Шановний Олегу Юрійовичу! Ми проаналізували тендер на капремонт житлового будинку по вул. Табірній у Києві. Ваша пропозиція була вигіднішою за переможця, але її відхилили через відсутність таблиці цін матеріалів та розрахунку заробітної плати. Оскільки це технічні документи, замовник не дав 24 години на виправлення, а відхилив пропозицію одразу. Ми створили AI-сервіс перевірки документів, який страхує від таких випадкових помилок перед подачею. Готові безкоштовно розібрати ваш наступний тендер. Коли зручно поспілкуватися?"
            else:
                msg_body = f"{greeting_str}! Ми проаналізували нещодавній тендер щодо {target['title'][:60]}... (ID: {target['prozorro_id']}). Помітили, що ваша ціна була вигіднішою за переможця на {target['diff_amount']:,.0f} грн, але пропозицію відхилили через помилки в довідках. Ми створили сервіс AI-аудиту документів для будівельників, який страхує від таких ловушок. Працюємо за моделлю Success Fee — 15% від маржі і лише після вашої реальної перемоги та укладення договору. Зробимо безкоштовний аналіз вашого наступного тендера?"
